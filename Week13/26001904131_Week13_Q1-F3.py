...
# -*- coding: utf-8 -*-
"""
Created on Sat Dec 21 19:50:29 2019
...
def errorFunction(x): # defining errorFunction (which is used as costFunction at function particleSwarmOptimization)                                  
    return sum([(15 * math.sin(x[i] / 5) ** 2) * 75 * math.cos(x[i] / 13) * 3 * math.sin(x[i] / 7) + x[i] ** 2 + 300 for i in range(len(x))])  #define as sum([(15 * math.sin(x[i] / 5) ** 2) * 75 * math.cos(x[i] / 13) * 3 * math.sin(x[i] / 7) + x[i] ** 2 + 300 for i in range(len(x))])  
